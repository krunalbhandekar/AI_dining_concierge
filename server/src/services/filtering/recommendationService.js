...
async function getRecommendations(criteria) {
  const fileStream = fs.createReadStream(restaurantsFile);
  ...
  for await (const line of rl) {
    ...
    // HARD FILTER
    const cityMatch = normalizeText(item.city) === normalizeText(criteria.area);
    const ratingMatch =
      criteria.minRating === null ||
      (item.rating !== null && item.rating >= criteria.minRating);

    if (!cityMatch || !ratingMatch) continue;

    // SOFT FILTER
    const budgetMatch =
      !criteria.budget || item.cost_bucket === criteria.budget;

    const cuisines = (item.cuisines || []).map(normalizeText);

    const cuisineMatch =
      criteria.cuisine.length === 0 ||
      criteria.cuisine.some((c) => cuisines.includes(c));

    if (!budgetMatch || !cuisineMatch) continue;
    ...
    for (const c of criteria.cuisine) {
      if (cuisines.includes(c)) score += 12;
    }
  ...
  // final sort
  candidates.sort(
    (a, b) => b.score - a.score || (b.rating || 0) - (a.rating || 0),
  );

  const result = diversifyRanked(candidates, 20);
